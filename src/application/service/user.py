...
class UserService:
    ...
    async def get_user_by_id(self, user_id: int) -> models.UserModel | None:
        return await self.repository.get_single(user_id=user_id)

    async def get_user_by_telegram_id(self, telegram_id: int) -> models.UserModel | None:
        return await self.repository.get_single(telegram_id=telegram_id)

    async def get_user_by_username(self, username: str) -> models.UserModel | None:
        return await self.repository.get_single(username=username)

    async def update_user(self, pk: int, user_in: dto.UserUpdate) -> models.UserModel:
